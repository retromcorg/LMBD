...
public class JBListener implements Listener, PoseidonCustomListener {
    ...
    public void onPlayerPreLoginEvent(final PlayerPreLoginEvent event) {
        ...
        ConnectionPause uuidCheckPause = event.addConnectionPause(plugin, "CheckUUIDBanStatus-" + event.getName());
        String uuidURL = plugin.getGetServerURL() + "/api/v1/isUUIDBanned?uuid=" + uuid.toString() + "&info=true";

        int[] checks = new int[2];
        checks[0] = Bukkit.getScheduler().scheduleAsyncDelayedTask(plugin, () -> {
            try {
                final JsonObject jsonObject = readJsonFromUrl(uuidURL);
                Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                    processResponse(event, uuid, plugin, jsonObject, "UUID");
                    event.removeConnectionPause(uuidCheckPause); //This can run before the other checks are done as we are now in the main thread.
                });
            } catch (Exception e) {
                plugin.getLog().log(Level.WARNING, "Invalid response, or connection error when checking for UUID ban", e);
                event.removeConnectionPause(uuidCheckPause);
            }
        });

        //Check if IP is banned
        ConnectionPause ipCheckPause = event.addConnectionPause(plugin, "CheckIPBanStatus-" + event.getName());
        String ipURL = plugin.getGetServerURL() + "/api/v1/isIPBanned?ip=" + event.getAddress().getHostAddress() + "&info=true";
        checks[1] = Bukkit.getScheduler().scheduleAsyncDelayedTask(plugin, () -> {
            try {
                final JsonObject jsonObject = readJsonFromUrl(ipURL);
                Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> {
                    processResponse(event, uuid, plugin, jsonObject, "IP");
                    event.removeConnectionPause(ipCheckPause);
                });
            } catch (Exception e) {
        ...
        //Timeout after 5 seconds - Even a request via a geosynchronous satellite could have been completed like 5 times by now.
        //This is cute, a mini watchdog.
        Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> {
            ...
            if (removedTask) {
                //This removes the connection pauses after the lookups are killed to allow the player to connect without lookup if the API is inaccessible.
                uuidCheckPause.removeConnectionPause();
                ipCheckPause.removeConnectionPause();
                plugin.logger(Level.WARNING, "At-least one of the lookup tasks for " + event.getName() + " hung likely due to a connectivity issue with the API. The tasks in question have been killed and the player has been allowed to join, however, this really is indicative of a major issue.");
            }
    ...
    public void processResponse(PlayerPreLoginEvent event, UUID uuid, JohnyBans plugin, JsonObject jsonObject, String banType) {
        ...
        if (JsonUtil.getBoolean(jsonObject, "isBanned", false)) {
            ...
            for (Ban ban : bans) {
                ...
                if (!evidenceBypass && !serverIgnoreBypass) {
                    ...
                    event.cancelPlayerLogin(ChatColor.RED + "You are banned. Visit: " + lmbdURL + "/b/" + ban.getBanID() + statisticsCode);
                    return;
                } else {
                    if (evidenceBypass) {
